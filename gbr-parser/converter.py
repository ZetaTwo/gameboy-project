...
def parse_tiles(gbr0):
    ...
    tiles = {gbr_object.object_id: {
                ...
                'palettes': None,
                'palette_maps': None,
            } for gbr_object in gbr0.objects if gbr_object.object_type == 0x0002}

    for gbr_object in gbr0.objects:
        if gbr_object.object_type == 0x0003:
            if tiles[gbr_object.body.tile_id]['settings']:
                raise ValueError('settings is already set for tile ID %d' % gbr_object.body.tile_id)
            tiles[gbr_object.body.tile_id]['settings'] = gbr_object.body
        elif gbr_object.object_type == 0x0004:
            if tiles[gbr_object.body.tile_id]['export']:
                raise ValueError('export is already set for tile ID %d' % gbr_object.body.tile_id)
            tiles[gbr_object.body.tile_id]['export'] = gbr_object.body
        elif gbr_object.object_type == 0x0005:
            if tiles[gbr_object.body.tile_id]['import']:
                raise ValueError('import is already set for tile ID %d' % gbr_object.body.tile_id)
            tiles[gbr_object.body.tile_id]['import'] = gbr_object.body
        elif gbr_object.object_type == 0x000D:
            if tiles[gbr_object.body.id]['palettes']:
                raise ValueError('palettes is already set for tile ID %d' % gbr_object.body.id)
            tiles[gbr_object.body.id]['palettes'] = gbr_object.body
        elif gbr_object.object_type == 0x000E:
            if tiles[gbr_object.body.id]['palette_maps']:
                raise ValueError('palette_maps is already set for tile ID %d' % gbr_object.body.id)
        ...
        for i in range(tile['data'].count):
            ...
            for y in range(tile['data'].height):
                for x in range(tile['data'].width):
                    color = convert_color(palette.colors[tile['data'].tiles[i][y*tile['data'].width+x]])
...
if __name__ == '__main__':
    ...
    for filename in files:
        with open('test/%s' % filename, 'rb') as fin:
            ...
            export_images('export/%s' % filename, images)
